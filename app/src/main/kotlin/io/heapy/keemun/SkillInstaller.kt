...
internal enum class SkillInstallAgent(val wireName: String) {
    CODEX("codex"),
    CLAUDE("claude"),
    ALL("all"),
    ;

    fun concreteAgents(): List<SkillInstallAgent> =
        when (this) {
            CODEX, CLAUDE -> listOf(this)
            ALL -> listOf(CODEX, CLAUDE)
        }

    companion object {
        fun parse(value: String): SkillInstallAgent =
            entries.firstOrNull { it.wireName == value.lowercase() }
                ?: throw IllegalArgumentException("--agent must be codex, claude, or all")
    }
}

internal enum class SkillInstallScope(val wireName: String) {
    PROJECT("project"),
    GLOBAL("global"),
    ;

    companion object {
        fun parse(value: String): SkillInstallScope =
            when (value.lowercase()) {
                PROJECT.wireName, "local" -> PROJECT
                GLOBAL.wireName, "user" -> GLOBAL
                else -> throw IllegalArgumentException("--scope must be project or global")
            }
    }
}

internal enum class SkillInstallStatus {
    INSTALLED,
    UPDATED,
    UNCHANGED,
}
...
internal data class SkillInstallResult(
    val agent: SkillInstallAgent,
    val path: KeemunPath,
    val status: SkillInstallStatus,
)

internal object SkillInstaller {
    fun install(
        agent: SkillInstallAgent,
        scope: SkillInstallScope,
        projectDir: KeemunPath,
        home: KeemunPath?,
        force: Boolean,
    ): List<SkillInstallResult> {
        val base = when (scope) {
            SkillInstallScope.PROJECT -> projectDir
            SkillInstallScope.GLOBAL -> home ?: KeemunUserHome?.let(::KeemunPath)
                ?: throw IllegalArgumentException("Cannot determine the user home directory; pass --home <dir>")
        }

        return agent.concreteAgents().map { concreteAgent ->
            installOne(concreteAgent, base, force)
        }
    }
    ...
    private fun installOne(
        ...
        val target = skillPath(base, agent)
        val exists = KeemunFiles.exists(target)
        if (exists) {
            val current = KeemunFiles.readString(target)
            if (current == keemunSkillContent) {
                return SkillInstallResult(agent, target, SkillInstallStatus.UNCHANGED)
            }
            if (!force) {
                throw IllegalArgumentException("$target already exists and differs; pass --force to overwrite it")
            }
        }

        target.parent?.let(KeemunFiles::createDirectories)
        KeemunFiles.writeString(target, keemunSkillContent)
        return SkillInstallResult(
            agent = agent,
            path = target,
            status = if (exists) SkillInstallStatus.UPDATED else SkillInstallStatus.INSTALLED,
        )
    }

    private fun skillPath(base: KeemunPath, agent: SkillInstallAgent): KeemunPath {
        val configDir = when (agent) {
            SkillInstallAgent.CODEX -> ".codex"
            SkillInstallAgent.CLAUDE -> ".claude"
            SkillInstallAgent.ALL -> error("all is not a concrete install target")
        }
...
private val keemunSkillContent: String =
    """
    ---
    name: keemun
